class Fleet :
    ...
    def append_spaceship(self, spaceships) :             # Vérifie que la capacité maximale de 15 vaisseaux est respecter
    ...
    def statistics(self) :
        print(f"\n--- Statistiques de la flotte {self.name}---")            # Affiche les statistiques de la flotte
        print(f"Nombre de vaisseaux : {len(self._spaceships)}")               # Nombres de vaisseaux de la flotte
        ...
        print("\nRépartition des rôles :")              # Permet d'afficher le nombre de personnes possédant le même rôle 
        ...
        if operators_count > 0 : 
            level_average = total_experience / operators_count                                     # Calcul du niveau moyen d'expérience des Opérateurs
            print(f"\nNiveau moyen d'expérience des opérateurs : {level_average: .2f}")             # Le ".2f" permet d'afficher un nombre avec deux chiffres après la virgule tous le temps
        else :
